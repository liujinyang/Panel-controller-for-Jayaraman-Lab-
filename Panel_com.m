...
switch lower(command)
    ...
    %add this command for Chuntao's rig    
    case 'set_laser_prob' 
        if ((~isequal(length(argument),1))||(~isnumeric(argument))||(argument(1) >100)||(argument(1) < 0))
            error('set laser prob command requires a numerical argument that is between 0 and 100');
        end
    
        send_serial( char([2 hex2dec('12') argument(1)]));
